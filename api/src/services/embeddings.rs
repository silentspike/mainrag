...
impl TeiClient {
    ...
    pub async fn embed(&self, text: &str) -> Result<Vec<f32>> {
        ...
        for attempt in 0..=MAX_RETRIES {
            ...
            match self.client.post(&url).json(&body).send().await {
                Ok(resp) if resp.status().is_success() => {
                    ...
                    metrics::histogram!("embedding_duration_seconds")
        ...
        Err(AppError::Tei(format!(
            "Embed failed after {} retries: {}",
    ...
    pub async fn embed_batch(&self, texts: &[&str]) -> Result<Vec<Vec<f32>>> {
        ...
        for attempt in 0..=MAX_RETRIES {
            ...
            match self.client.post(&url).json(&body).send().await {
                Ok(resp) if resp.status().is_success() => {
                    let embeddings: Vec<Vec<f32>> = resp.json().await.map_err(|e| {
                        AppError::Tei(format!("Failed to parse batch response: {}", e))
                    })?;
        ...
        metrics::counter!("tei_retries_exhausted").increment(1);
        Err(AppError::Tei(format!(
            "Batch embed failed after {} retries: {}",
            MAX_RETRIES, last_err
        )))
    }
}
