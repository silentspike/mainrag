-- Migration 064: bind each unchanged or rebuilt final candidate to its own
-- qualification commit while retaining the exact installed runtime commit in
-- the reviewed activation manifest. This replacement is additive to 057.
...
BEGIN
    ...
    LOOP
        IF jsonb_typeof(v_entry) <> 'object'
           ...
           OR COALESCE(v_entry ->> 'candidate_commit_sha', '') !~ '^[0-9a-f]{40}$'
        ...
        IF NOT EXISTS (
            ...
            WHERE source.id = v_source_id
              ...
              AND evidence.commit_sha = v_entry ->> 'candidate_commit_sha'
        ) THEN
